...
public static Optional<Student> findByIndexNumber(final int indexNumber) {
    try {
        var result = QueryExecutor.read(
            "SELECT * FROM student WHERE index_number = ?", indexNumber
        );
    ...
    public Map<Course, Float> createReport() {
        String sql = "SELECT c.id, c.name, AVG(g.grade) as avg_grade FROM grade AS g " +
        ...
        try {
            ...
            while(rs.next()) {
                var course = new Course(rs.getInt(Course.Columns.ID), rs.getString(Course.Columns.NAME));
                float grade = rs.getFloat("avg_grade");
